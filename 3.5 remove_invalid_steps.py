...
base_path = "stabletoolbench/data_eval/answer/new_train_1229"
...
1. Determine the `answer_status` of the answer by following these rules:
   ...
   - If all API Observation messages indicate errors occurred, return "Failed".
   - If you find the information in the "final_answer" is not true/valid according to the messages in API Observation, return "Failed".
...
2. Analyze the reasoning process to determine if there are any irrelevant steps or incorrect attempts that do not contribute to the final result:
   - If all steps in the reasoning process are meaningful and contribute to solving the query, set `steps_validity` to "True".
   - If there are irrelevant or incorrect steps that do not contribute to solving the query, set `steps_validity` to "False".

Output your thought in "content" and include `answer_status` and `steps_validity` in the JSON output.
