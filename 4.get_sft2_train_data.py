                ...
                try:
                    with open(file_path, 'r') as f:
                        ...
                        if data.get('win') == True and (data.get('answer_status').get('answer_status') == "Pass" or data.get('answer_status').get('answer_status') == "Solved") and (data.get('answer_status').get('all_steps_validity') == "yes" or data.get('answer_status').get('all_steps_validity') == True or data.get('answer_status').get('all_steps_validity') == "True" or data.get('answer_status').get('all_steps_validity') == "true"):
                            ...
                            wrong_action = "<thought>\n" + data["wrong_iteration"]["action"]["thought"] + "\n</thought>\n<execute>\n" + data["wrong_iteration"]["action"]["execute"] + "\n</execute>"
                            wrong_message = [{"role": "assistant", "content": wrong_action}]
                            observation = data["wrong_iteration"]["observation"]
                            if "Observation:\nError: Failed to run the code after 3 attempts.\nError: Traceback (" in observation:
                                continue
                            if observation == "":
                                continue
                            if "{'error': '', 'response': '{}'}" in observation:
                                continue
                            if "print" not in data["wrong_iteration"]["action"]["execute"]:
                                continue
                            quality_score = str(data["reflextion_data_quality"]["score"])

                            #! 只保留quality_score>=8
                            if quality_score < "8":
                                continue
                            elif quality_score == "8" and group == "G1":
                                # 1/2
                                if random.random() > 0.3:
                                    continue
                            elif quality_score == "8" and group == "G3":
                                # 1/3
                                if random.random() < 0.3:
                                    continue
                            if quality_score == "8":
                                quality_8_json[group] += 1
                            if quality_score not in quality_score_json:
                                quality_score_json[quality_score] = 0
                            quality_score_json[quality_score] += 1

                            
                            


                            pre_messages = pre_messages + wrong_message + [{"role": "user", "content": "Observation:\n" + observation}]
                            right_action = "<thought>\n" + data["wrong_iteration"]["reflection_iteration"]["action"]["thought"] + "\n</thought>\n<execute>\n" + data["wrong_iteration"]["reflection_iteration"]["action"]["execute"] + "\n</execute>"
                            for i in range(len(pre_messages)):
                                if pre_messages[i]["role"] == "assistant":
                                    pre_messages[i]["no_loss"] = True
                            all_messages = pre_messages + [{"role": "assistant", "content": right_action,"no_loss": False}]
                            all_messages[0]["content"] = replace_system_prompt(all_messages[0]["content"])
                            train_data.append({"id": str(group)+"@"+id + "_reflection", "messages": all_messages})
                except:
                    continue

# 统计quality
print(quality_score_json)
print(quality_8_json)
# 随机选择10% train_data
...
with open("data/training_data/0108_10_training_data.json", "r") as f:
    train_data_old = json.load(f)
    for data in train_data_old:
        for i in range(len(data["messages"])):
            if data["messages"][i]["role"] == "assistant":
                data["messages"][i]["no_loss"] = False
    train_data = train_data + train_data_old
...
with open(f"data/training_data/0113_15_training_data_reflextion.json", "w") as f:
